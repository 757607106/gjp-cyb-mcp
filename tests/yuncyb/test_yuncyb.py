...
from yuncyb.catalog import normalize_live_product_rows
from yuncyb.config import YunCybSettings
from yuncyb.models import Product
...
from yuncyb.session import YunCybSession, parse_order_text
from yuncyb.toolset import YUNCYB_MCP_TOOL_NAMES, YunCybToolSet
...
def _yuncyb_toolset(session, api=None):
...
def test_spoken_alias_phrase_matches_erp_product(tmp_path):
    ...
    result = asyncio.run(_yuncyb_toolset(session).preview_sales_order(
        "来十斤马铃薯",
        source="voice",
    ))
...
def test_create_draft_puts_best_match_outside_and_similar_products_inside(
    ...
    result = asyncio.run(_yuncyb_toolset(session).preview_sales_order("牛肉10斤"))
...
def test_create_draft_returns_unmatched_products_separately(tmp_path):
    ...
    result = asyncio.run(_yuncyb_toolset(session).preview_sales_order("量子芯片2箱"))
...
def test_create_draft_validates_frontend_confirmation(tmp_path):
    ...
    toolset = _yuncyb_toolset(session)
...
def test_yuncyb_toolset_exposes_complete_sales_order_tools(tmp_path):
    session = _session(
        tmp_path,
        [{"ptypeid": "P001", "pfullname": "土豆", "unit": "斤"}],
    )
    ...
    assert {tool.name for tool in toolset.tools()} == set(YUNCYB_MCP_TOOL_NAMES)
    ...
    assert toolset.get("search_business_references").is_read_only is True
...
def test_yuncyb_settings_contain_no_runtime_output_path():
    assert {item.name for item in fields(YunCybSettings)} == {
...
def test_sync_products_replaces_in_memory_catalog_without_writing_file(tmp_path):
    session = YunCybSession.from_settings(
        _settings(tmp_path),
    ...
    class FakeYunCybApi:
        async def fetch_products(self, context, limit=None):
            assert context.account_id == "yuncyb-test"
            assert limit == 1
            return YunCybProductSnapshot(
                products=(
                    {
                        "productId": "P001",
                        "code": "SP001",
    ...
    result = asyncio.run(_yuncyb_toolset(session, FakeYunCybApi()).sync_products(limit=1))
...
def test_in_memory_sync_preserves_aliases_replaces_rows_and_is_not_shared(
    ...
    toolset = _yuncyb_toolset(session, SequenceYunCybApi())
    ...
    new_session = YunCybSession.from_settings(
        _settings(tmp_path),
        allow_missing_catalog=True,
    )
...
def test_failed_sync_keeps_previous_in_memory_catalog(tmp_path):
    ...
    toolset = _yuncyb_toolset(session, FailingSecondSyncApi())
...
def test_create_draft_returns_error_when_auto_sync_fails(tmp_path):
    ...
    result = asyncio.run(_yuncyb_toolset(session).preview_sales_order("土豆2斤"))

    assert result["ok"] is False
    assert result["error"]["code"] == "yuncyb_api_not_configured"
...
class CompleteSalesOrderApi:
    ...
    async def create_sales_order(self, context, payload):
        ...
        return YunCybSalesOrderResult(order_id="SO-20260804-1")

    @staticmethod
    async def get_sales_order_detail(context, order_id):
        return YunCybSalesOrderDetailResult(
            order={
                "id": order_id,
    ...
    async def search_sales_orders(
        ...
        *,
        ...
        return YunCybSalesOrderPageResult(
            total=1,
            page_num=page_num,
            page_size=page_size,
            orders=(
    ...
    async def update_sales_order(self, context, order_id, payload):
        ...
        return YunCybSalesOrderResult(order_id=order_id)
...
def test_preview_sales_order_returns_missing_fields_and_actions(tmp_path):
    ...
    result = asyncio.run(_yuncyb_toolset(session).preview_sales_order(order_text="土豆2斤"))
...
def test_sales_order_preview_rounds_amounts_per_line(tmp_path):
    ...
    prepared = asyncio.run(_yuncyb_toolset(
...
def test_reference_outputs_preserve_internal_ids(tmp_path):
    ...
    toolset = _yuncyb_toolset(session, CompleteSalesOrderApi())

    options = asyncio.run(toolset.search_business_references("customer", "客户甲"))
...
def test_reference_candidates_are_ranked_by_match_quality(tmp_path):
    ...
    result = asyncio.run(_yuncyb_toolset(
        session, RankedReferenceApi(),
    ).search_business_references("customer", "客户甲"))
...
class _RecordingReferenceApi:
    """记录 search_business_references 翻页与自动同步限额的调用参数。"""
    ...
    async def fetch_products(self, context, limit=None):
        ...
        return YunCybProductSnapshot(products=tuple(self.products))
    ...
    async def create_sales_order(self, context, payload):
        return YunCybSalesOrderResult(order_id="SO-1")

    async def get_sales_order_detail(self, context, order_id):
        return YunCybSalesOrderDetailResult(order={"orderNo": "SO20260804001"})
...
def test_search_business_references_passes_page_to_api(tmp_path):
    """search_business_references 应把页码透传给基础资料 API。"""
    session = _session(tmp_path, [{"id": "P001", "name": "土豆", "unit": "斤"}])
    api = _RecordingReferenceApi([{"id": "P001", "name": "土豆", "unit": "斤"}])
    toolset = _yuncyb_toolset(session, api)

    result = asyncio.run(toolset.search_business_references(
...
def test_match_logger_records_user_confirmed_lines(tmp_path):
    ...
    asyncio.run(_yuncyb_toolset(session).preview_sales_order(
        "牛肉10斤",
        confirmed_products=[{"line_id": "L001", "product_id": "P002"}],
    ))
...
def test_create_match_logger_from_env_returns_null_when_unset(monkeypatch):
    ...
    monkeypatch.delenv("YUNCYB_MATCH_LOG", raising=False)
...
def test_list_products_auto_syncs_when_empty(tmp_path):
    ...
    toolset = _yuncyb_toolset(session, FakeApi())
    result = asyncio.run(toolset.list_products())
...
def test_sync_products_returns_sample_products(tmp_path):
    ...
    class SampleApi:
        @staticmethod
        async def fetch_products(context, limit=None):
            return YunCybProductSnapshot(products=tuple(products_data))

    toolset = _yuncyb_toolset(session, SampleApi())
...
def test_reference_variants_keep_distinct_ids(tmp_path):
    ...
    toolset = _yuncyb_toolset(session, MultiVariantApi())
    result = asyncio.run(toolset.preview_sales_order(
        order_text="土豆2斤",
        customer="好又多超市西湖店",
        warehouse="一号仓",
...
def test_yuncyb_tools_carry_output_schema(tmp_path):
    ...
    assert set(by_name) == set(YUNCYB_MCP_TOOL_NAMES)
    for name in YUNCYB_MCP_TOOL_NAMES:
...
def test_tool_outputs_validate_against_output_schema(tmp_path):
    ...
    toolset = _yuncyb_toolset(session, YunCybApi())
    by_name = {tool.name: tool for tool in toolset.tools()}
    ...
    options = asyncio.run(toolset.search_business_references("customer", "客户甲"))
    jsonschema.validate(options, by_name["search_business_references"].output_schema)
...
def test_update_sales_order_rejects_unmatched_reference_name(tmp_path):
    ...
    toolset = _yuncyb_toolset(session, NoStaffApi())
...
def test_update_sales_order_rejects_ambiguous_reference_name(tmp_path):
    ...
    toolset = _yuncyb_toolset(session, AmbiguousStaffApi())
