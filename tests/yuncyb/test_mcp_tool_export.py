...
YUNCYB_MCP_TOOL_NAMES 是开单工具的完整白名单，经转换后必须等于
...
from yuncyb.adapters import UnavailableYunCybApi
from yuncyb.config import YunCybSettings
from yuncyb.mcp_service import YUNCYB_MCP_SERVER_NAME
from yuncyb.presentation import filter_yuncyb_result, present_yuncyb_result
...
class _StaticToolSetResolver:
    def __init__(self, toolset: YunCybToolSet) -> None:
        self._toolset = toolset

    def resolve(self, _context: InvocationContext) -> YunCybToolSet:
        return self._toolset


def test_yuncyb_mcp_tool_names_all_map_to_camelcase() -> None:
    """开单白名单内每个工具名都能被导出层转为 camelCase。"""
    actual = {_snake_to_camel(name) for name in YUNCYB_MCP_TOOL_NAMES}
    assert actual == _EXPECTED_CAMEL


def test_yuncyb_mcp_tool_names_remain_snake_case_internally() -> None:
    """Python 函数名（tool.name）保持 snake_case，转换只在导出层发生。"""
    for name in YUNCYB_MCP_TOOL_NAMES:
        assert "_" in name, "内部工具名应保持 snake_case：%s" % name


def test_public_mcp_server_name_is_yuncyb() -> None:
    assert YUNCYB_MCP_SERVER_NAME == "yuncyb"


def _make_yuncyb_toolset(tmp_path) -> YunCybToolSet:
...
def test_create_mcp_server_lists_tools_in_camelcase(tmp_path) -> None:
    ...
    """
    toolset = _make_yuncyb_toolset(tmp_path)
    context = InvocationContext(
        tenant_id="tenant-test",
        subject_id="user-test",
        account_id="yuncyb-test",
...
def _filtered_success_payload(name):
    ...
    elif name == "search_business_references":
        fields = {
...
@pytest.mark.parametrize("name", sorted(YUNCYB_MCP_TOOL_NAMES))
...
def test_filtered_output_schemas_accept_all_59_tools(tmp_path, name, outcome):
    ...
    assert len(YUNCYB_MCP_TOOL_NAMES) == 59
    tools = _make_yuncyb_toolset(tmp_path)
    ...
    for tool_name in (name, _snake_to_camel(name)):
        filtered = filter_yuncyb_result(tool_name, payload)
        ...
        text, structured = present_yuncyb_result(tool_name, payload)
...
def test_all_published_tools_are_self_describing_without_business_instructions(tmp_path):
    tools = _make_yuncyb_toolset(tmp_path)
    context = InvocationContext("tenant-test", "user-test", "yuncyb-test")
    server = create_mcp_server(
    ...
    for tool in published:
        ...
        for internal_name in YUNCYB_MCP_TOOL_NAMES:
