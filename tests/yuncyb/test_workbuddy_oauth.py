...
from yuncyb.workbuddy_oauth import (
...
def _register(client: TestClient, settings: WorkBuddyOAuthSettings) -> str:
    response = client.post(
        ...
        json={
            ...
            "response_types": ["code"],
            "scope": "yuncyb:read yuncyb:write",
...
def test_workbuddy_oauth_discovery_and_full_pkce_flow():
    ...
    try:
        ...
        assert tokens["scope"] == "yuncyb:read yuncyb:write"
...
def test_workbuddy_app_keeps_health_public_and_mcp_protected():
    settings = _settings()
    store = EncryptedSqliteOAuthStore(settings.database_path, settings.encryption_key)
    app = create_workbuddy_yuncyb_app(
...
def test_workbuddy_connector_assets_keep_auth_modes_separate():
    ...
    oauth_meta = json.loads((root / "yuncyb" / "connector-meta.json").read_text())
    oauth_mcp = json.loads((root / "yuncyb" / "mcp.json").read_text())
    token_meta = json.loads((root / "yuncyb-token" / "connector-meta.json").read_text())
    token_mcp = json.loads((root / "yuncyb-token" / "mcp.json").read_text())

    assert oauth_meta["source"] == "yuncyb"
    assert "auth_mode" not in oauth_meta
    assert "headers" not in oauth_mcp["mcpServers"]["yuncyb"]
    assert token_meta["source"] == "yuncyb-token"
    assert token_meta["auth_mode"] == "token"
    assert token_mcp["mcpServers"]["yuncyb"]["headers"] == {"X-API-Key": "${ERP_API_KEY}"}


def test_workbuddy_skill_allowed_tools_match_published_whitelist():
    """SKILL.md 的 allowed-tools 必须与服务端发布白名单保持一致。"""
    from yuncyb.toolset import YUNCYB_MCP_TOOL_NAMES
    from gjp_common.mcp import _snake_to_camel

    skill = (
        Path(__file__).resolve().parents[2]
        / "integrations" / "workbuddy" / "yuncyb" / "skills" / "yuncyb" / "SKILL.md"
    ...
    expected = frozenset(_snake_to_camel(name) for name in YUNCYB_MCP_TOOL_NAMES)
