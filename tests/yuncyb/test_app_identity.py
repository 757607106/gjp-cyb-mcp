...
def test_resolver_partitions_session_by_conversation_id() -> None:
    ...
    assert context_a.session_id == "yuncyb-user-1-conv-a"
    assert context_b.session_id == "yuncyb-user-1-conv-b"
    assert context_a.session_id != context_b.session_id
    # 未传对话标识时退化为按登录账号隔离，保持既有行为
    assert context_default.session_id == "yuncyb-user-1"
    # 三个会话各自持有独立凭据条目
...
def test_resolver_clips_overlong_conversation_id() -> None:
    ...
    assert context.session_id == "yuncyb-u-" + "c" * 64
...
def test_api_key_resolver_partitions_session_by_conversation_id() -> None:
    ...
    assert context.session_id == "yuncyb-apikey-conv-a"
...
def test_resolver_shares_catalog_state_within_tenant(
    ...
    from yuncyb.app import YunCybSessionToolSetResolver
    from yuncyb.config import YunCybSettings

    monkeypatch.setenv("YUNCYB_BASE_URL", "https://erp.example/api")
    ...
    resolver = YunCybSessionToolSetResolver(
        SessionCredentialStore(),
        settings,
        30.0,
    )
...
def test_production_without_secret_allows_api_key_and_bearer(
    ...
    monkeypatch.setenv("GJP_ENV", "production")
    monkeypatch.delenv("YUNCYB_API_KEYS", raising=False)
