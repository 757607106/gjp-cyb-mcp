...
from yuncyb.session import YunCybSession, _normalize_confirmed_products
...
def _toolset(tmp_path, api):
    ...
    context = InvocationContext(
        ...
        account_id="yuncyb-bug-hunt",
        session_id="session-bug-hunt",
        scopes=frozenset({"yuncyb:read", "yuncyb:write"}),
    )
    return YunCybToolSet(session, api, InvocationContextStore(default=context))


class RecordingYunCybApi:
    ...
    async def create_sales_order(self, context, payload):
        ...
        return YunCybSalesOrderResult(order_id="1001")

    async def get_sales_order_detail(self, context, order_id):
        self.detail_queries.append(order_id)
        return YunCybSalesOrderDetailResult(order={"orderNo": "XS1"})

    async def search_sales_orders(self, context, **kwargs):
        from yuncyb.ports import YunCybSalesOrderPageResult
...
def test_preview_rejects_zero_quantity(tmp_path):
    ...
    result = asyncio.run(
        _toolset(tmp_path, RecordingYunCybApi()).preview_sales_order(
...
def test_update_rejects_zero_quantity_for_contrast(tmp_path):
    ...
    result = asyncio.run(
        _toolset(tmp_path, RecordingYunCybApi()).update_sales_order(
            order_id="1001",
            order_date="2026-08-28",
            handler_id="张三",
...
def test_current_behavior_sync_products_zero_limit_error_message(tmp_path):
    ...
    result = asyncio.run(
        _toolset(tmp_path, RecordingYunCybApi()).sync_products(limit=0),
    )
