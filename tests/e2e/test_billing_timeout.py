...
复用主 e2e 的环境变量开关（ERP_BILLING_E2E_API_KEY），独立启动
第二个服务实例，把 ERP_BILLING_BASE_URL 指向不可达地址并收紧
超时预算，覆盖"等待"场景：

- 连接被拒（本地无监听端口）快速失败；
- 黑洞地址（RFC 5737 保留网段）等待至 ERP_BILLING_TIMEOUT_SECONDS
  配置的超时后受控失败，不会无限等待；
...
pytestmark = pytest.mark.skipif(
    not _API_KEY,
    reason="需要 ERP_BILLING_E2E_API_KEY 环境变量",
)
...
def test_blackhole_upstream_times_out_within_budget(blackhole_url):
    """黑洞地址按 ERP_BILLING_TIMEOUT_SECONDS 受控超时，不会无限等待。"""
    started = time.perf_counter()
