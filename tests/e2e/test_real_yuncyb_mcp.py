...
默认整体跳过，同时设置以下环境变量后启用（上游为真实 ERP 测试环境）：
    ...
    YUNCYB_E2E_BEARER_TOKEN=<ERP JWT>  # 与 API Key 二选一
...
_BEARER_TOKEN = os.environ.get("YUNCYB_E2E_BEARER_TOKEN", "").strip()
_ERP_BASE_URL = os.environ.get("YUNCYB_E2E_BASE_URL", "").strip()

pytestmark = pytest.mark.skipif(
    not (_API_KEY or _BEARER_TOKEN) or not _ERP_BASE_URL,
    reason="需要 YUNCYB_E2E_API_KEY/YUNCYB_E2E_BEARER_TOKEN 与 YUNCYB_E2E_BASE_URL 环境变量",
)
...
def server_url():
    ...
    port = _free_port()
    env = os.environ.copy()
    env["YUNCYB_BASE_URL"] = _ERP_BASE_URL
    env["GJP_ENV"] = "local"
    env.pop("YUNCYB_JWT_SECRET", None)
...
def yuncyb_data(server_url):
    ...
    warehouses = _call(
        "searchBusinessReferences", {"reference_type": "warehouse", "limit": 5},
        record_timing=False,
    )
...
def test_initialize_lists_all_tools(server_url):
    ...
    assert names == {
        # 商品目录与基础资料
        "syncProducts", "listProducts", "searchProducts", "searchBusinessReferences",
        # 销售单
...
def test_search_business_references_with_page(server_url):
    """searchBusinessReferences 支持翻页并保留候选唯一身份。"""
    page1 = _call("searchBusinessReferences", {
    ...
    page2 = _call("searchBusinessReferences", {
    ...
    for reference_type in ("warehouse", "handler"):
        result = _call("searchBusinessReferences", {
...
def test_search_business_references_keyword_filter(server_url):
    """searchBusinessReferences 关键词过滤应命中包含该词的候选。"""
...
def test_search_business_references_invalid_type(server_url):
    """reference_type 非法时被输入 Schema 枚举在协议层拦截，不产生 ERP 调用。"""
    error_text = _call_protocol_error("searchBusinessReferences", {
...
def test_search_business_references_supplier_and_settlement_account(server_url):
    """供应商与结算账户资料类型可查询并分页。"""
...
def test_search_business_references_limit_boundaries(server_url):
    ...
    for arguments in (
        ...
        error_text = _call_protocol_error("searchBusinessReferences", arguments)
        assert "请求格式不正确" in error_text, arguments
