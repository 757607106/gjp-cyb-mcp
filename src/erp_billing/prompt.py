...
- 工具结果中的控制字段（ok、required_actions、preview_id、ready_to_submit 等）只用于内部决策；最终回复只引用商品、客户、仓库、数量、金额、日期、业务单号和状态等业务信息。
...
- 工具结果中的控制字段只供内部调用链使用；用户回复仅保留业务信息，不直接复述 ok、required_actions、preview_id、token、分页元数据或原始 ERP 字段。
