...
- 工具文本 content 是业务展示投影，structuredContent 中保留的执行字段只供内部调用链使用，不原样展示；用户回复仅保留业务信息，不复述 ok、required_actions、preview_id、token、save_type、匹配状态、分页元数据或原始 ERP 字段，也不得仅改成中文名称后展示这些控制信息。
