...
class ErpAuthenticatedHttpAdapter:
    ...
    async def fetch_products(
        ...
        return YunCybProductSnapshot(products=tuple(products))
    ...
    async def get_sales_order_detail(
        ...
        return YunCybSalesOrderDetailResult(order=order)
    ...
    async def update_sales_order(
        ...
        if not result_id:
            result_id = resolved
        return YunCybSalesOrderResult(order_id=result_id)
    ...
    async def _resolve_document_id(
        ...
        token: str,
        *,
        search: Callable[[str], Awaitable[YunCybDocumentPageResult]],
        number_field: str,
        code_prefix: str,
        doc_label: str,
    ) -> str:
    ...
    async def _get_document(
        ...
        return YunCybDocumentDetailResult(document=document)
    ...
    async def _create_document(
        ...
        return YunCybDocumentResult(document_id=document_id)
    ...
    @staticmethod
    def _data_result(data: dict, label: str) -> YunCybDataResult:
        payload = data.get("data")
        if payload is None:
            raise DomainError("erp_live_response_invalid", "ERP %s数据缺失" % label)
        return YunCybDataResult(data=payload)
    ...
    async def _resolve_purchase_order_id(
        ...
        async def search(token: str) -> YunCybDocumentPageResult:
    ...
    async def update_purchase_order(
        ...
        return YunCybDocumentResult(document_id=result_id)
    ...
    async def _request_json(
        ...
        *,
        ...
        if credential.kind == "bearer":
            ...
            raise DomainError("yuncyb_api_unauthorized", "当前开单会话的鉴权类型无效")
...
class UnavailableYunCybApi:
    """本地参考客户端未注入开单产品 Adapter 时返回明确错误。"""

    @staticmethod
    def _raise() -> NoReturn:
        raise DomainError("yuncyb_api_not_configured", "开单服务尚未注入已鉴权 YunCybApiPort")
