...
保存过账，不暴露草稿态。实现为 YunCybToolSet 的混入类，与销售单工具
...
_PREVIEW_PURCHASE_ORDER_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "supplier": {"type": "string", "description": "采购供应商，业务必填；名称、编号或 searchBusinessReferences 返回的供应商 ID。"},
        "warehouse": {"type": "string", "description": "采购入库仓库，业务必填；名称、编号或 searchBusinessReferences 返回的仓库 ID。"},
        "handler": {"type": "string", "description": "采购经手人，业务必填；名称、编号或 searchBusinessReferences 返回的经手人 ID。"},
...
_LIST_PURCHASE_ORDERS_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "supplier_id": {"type": "string", "description": "供应商内部 ID 或名称，候选通过 searchBusinessReferences 查询；留空查询所有供应商。"},
    },
}
...
_UPDATE_PURCHASE_ORDER_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "handler_id": {"type": "string", "description": "新经手人的内部 ID 或名称，取自 searchBusinessReferences；省略保留，传入时不能为空。"},
        "supplier_id": {"type": "string", "description": "新供应商的内部 ID 或名称，取自 searchBusinessReferences；留空保留原供应商。"},
        "warehouse_id": {"type": "string", "description": "新入库仓库的内部 ID 或名称，取自 searchBusinessReferences；留空保留原仓库。"},
        "items": {
            "type": "array",
        ...
        "discount_account_id": {"type": "string", "description": "优惠承担结算账户的内部 ID，取自 searchBusinessReferences；直接传 ID，不解析名称，留空不修改。"},
        "payment_amount": {"type": "number", "description": "修改后的采购单付款金额，非负，允许 0；省略保留。仅追加一笔付款请用 previewPurchasePayment。"},
        "payment_account_id": {"type": "string", "description": "付款结算账户内部 ID，取自 searchBusinessReferences；直接传 ID，不解析名称，留空不修改。"},
...
_PREVIEW_PURCHASE_RETURN_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "refund_account_id": {"type": "string", "description": "接收供应商退款的结算账户名称、编号或 ID，候选用 searchBusinessReferences；退款大于 0 时必填并须唯一匹配。"},
        "discount_amount": {"type": "number", "minimum": 0, "description": "本次采购退货优惠/减免金额，非负；省略或 0 不附加优惠。与退款同时提供时合计不得超过退货总额。"},
        "discount_account_id": {"type": "string", "description": "优惠承担结算账户名称、编号或 ID，候选用 searchBusinessReferences；优惠大于 0 时必填并须唯一匹配。"},
...
_PREVIEW_SALES_RETURN_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "refund_account_id": {"type": "string", "description": "向客户退款的结算账户名称、编号或 ID，候选用 searchBusinessReferences；退款大于 0 时必填并须唯一匹配。"},
        "discount_amount": {"type": "number", "minimum": 0, "description": "本次销售退货折让金额，非负；省略或 0 不附加折让。与退款同时提供时合计不得超过退货总额。"},
        "discount_account_id": {"type": "string", "description": "折让承担结算账户名称、编号或 ID，候选用 searchBusinessReferences；折让大于 0 时必填并须唯一匹配。"},
...
_LIST_SALES_RETURNS_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "customer_id": {"type": "string", "description": "客户内部 ID 或名称，候选通过 searchBusinessReferences 查询；留空查询所有客户。"},
    },
}
...
_PREVIEW_SALES_RECEIPT_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "receipt_account": {"type": "string", "description": "收款结算账户名称、编号或 searchBusinessReferences 返回的 ID；即使收款为 0、仅免账也须提供并匹配。"},
        "discount_amount": {"type": "number", "minimum": 0, "description": "本次免账金额（优惠/折让），非负，省略按 0；与收款合计须大于 0 且不超过该单未收金额。"},
        "discount_account": {"type": "string", "description": "免账承担结算账户名称、编号或 searchBusinessReferences 返回的 ID；免账大于 0 时必填并须匹配。"},
    },
...
_PREVIEW_PURCHASE_PAYMENT_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "payment_account": {"type": "string", "description": "付款结算账户名称、编号或 searchBusinessReferences 返回的 ID；即使付款为 0、仅免账也须提供并匹配。"},
...
_PREVIEW_STOCK_TRANSFER_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "from_warehouse": {"type": "string", "description": "必填，调出仓库名称、编号或 searchBusinessReferences 返回的仓库 ID，须与调入仓库不同。"},
        "to_warehouse": {"type": "string", "description": "必填，调入仓库名称、编号或 searchBusinessReferences 返回的仓库 ID，须与调出仓库不同。"},
        "handler": {"type": "string", "description": "必填，调拨经手人名称、编号或 searchBusinessReferences 返回的经手人 ID。"},
...
_PREVIEW_OTHER_STOCK_DOC_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "warehouse": {"type": "string", "description": "必填，发生其他出入库的仓库名称、编号或 searchBusinessReferences 返回的仓库 ID。"},
        "handler": {"type": "string", "description": "必填，单据经手人名称、编号或 searchBusinessReferences 返回的经手人 ID。"},
...
_PREVIEW_RECEIPT_ORDER_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "account": {"type": "string", "description": "必填，收款结算账户名称、编号或 searchBusinessReferences 返回的账户 ID。"},
        "handler": {"type": "string", "description": "必填，收款经手人名称、编号或 searchBusinessReferences 返回的经手人 ID。"},
        "customer": {"type": "string", "description": "收款往来客户名称或 searchBusinessReferences 返回的 ID；核销销售类单据时必填并须匹配，与 supplier 只能提供其一。"},
        "supplier": {"type": "string", "description": "收款往来供应商名称或 searchBusinessReferences 返回的 ID；核销采购类单据（如采购退货）时必填并须匹配，与 customer 只能提供其一。"},
        ...
        "discount_account": {"type": "string", "description": "免账承担结算账户名称、编号或 searchBusinessReferences 返回的 ID；优惠大于 0 时必填并须匹配。"},
        "fund_type": {
            "type": "string",
...
_PREVIEW_PAYMENT_ORDER_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "account": {"type": "string", "description": "必填，付款结算账户名称、编号或 searchBusinessReferences 返回的账户 ID。"},
        "handler": {"type": "string", "description": "必填，付款经手人名称、编号或 searchBusinessReferences 返回的经手人 ID。"},
        "supplier": {"type": "string", "description": "付款往来供应商名称或 searchBusinessReferences 返回的 ID；核销采购类单据时必填并须匹配，与 customer 只能提供其一。"},
        "customer": {"type": "string", "description": "付款往来客户名称或 searchBusinessReferences 返回的 ID；核销销售类单据（如销售退货）时必填并须匹配，与 supplier 只能提供其一。"},
...
_LIST_FINANCIAL_ORDERS_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "counterparty_id": {"type": "string", "description": "往来客户或供应商的内部 ID、名称，候选用 searchBusinessReferences；先尝试匹配客户再供应商，留空不筛选。"},
        "account_id": {"type": "string", "description": "结算账户内部 ID 或名称，候选用 searchBusinessReferences；留空不按账户筛选。"},
    },
}
...
class DocumentTools:
    ...
    以混入类并入 YunCybToolSet：session、_api、_contexts、基础资料解析
    ...
    async def preview_purchase_order(
        ...
        基础资料用 searchBusinessReferences，商品用 searchProducts 查候选。
        ...
        try:
            ...
            product_payload = (
                draft.business_products_payload()
    ...
    async def void_purchase_order(
        ...
        try:
            context = self._contexts.get()
            context.require_scope("yuncyb:write")
    ...
    async def preview_stock_transfer(
        ...
        仓库和经手人候选用 searchBusinessReferences，商品候选用 searchProducts。
    ...
    async def preview_other_stock_doc(
        ...
        先用 listStockDocTypes 查询方向对应的类型；基础资料用 searchBusinessReferences，
    ...
    async def preview_receipt_order(
        ...
        核实单据、往来单位及内部 ID；基础资料用 searchBusinessReferences 查候选。
