"""管家婆云创业版独立 MCP 服务，只发布 YunCybToolSet。"""
...
from .prompt import YUNCYB_MCP_INSTRUCTIONS
from .presentation import present_yuncyb_result
from .toolset import YunCybToolSet

YUNCYB_MCP_SERVER_NAME = "yuncyb"


def rate_limit_yuncyb_mcp(app: ASGIApp) -> ASGIApp:
...
def create_yuncyb_mcp_service(
    ...
    """创建管家婆云创业版 MCP 服务；部署时使用独立域名、进程和认证配置。"""
    if not isinstance(schema_toolset, YunCybToolSet):
        raise TypeError("管家婆云创业版 MCP 只能发布 YunCybToolSet")
    server = create_mcp_server(
        YUNCYB_MCP_SERVER_NAME,
        schema_toolset,
        identity_resolver,
        toolset_resolver,
        instructions=YUNCYB_MCP_INSTRUCTIONS,
