...
运行方式：
    ...
    uv run uvicorn yuncyb.workbuddy_app:app --host 0.0.0.0 --port 8103

本入口提供生产 WorkBuddy OAuth、ERP AI Token 绑定和 HTTP 保护层；独立的
``yuncyb.app:app`` 提供生产 ``X-API-Key`` 直连。
"""
...
from .adapters import UnavailableYunCybApi, create_match_logger_from_env
from .app import YunCybSessionToolSetResolver
...
__all__ = ["app", "create_workbuddy_yuncyb_app"]


def create_workbuddy_yuncyb_app(
    *,
    ...
    erp_settings = YunCybSettings.from_env()
    validator = binding_validator
    if validator is None:
        base_url = get_env_value("YUNCYB_BASE_URL").strip()
        if not base_url:
            raise DomainError("business_connection_invalid", "未配置 YUNCYB_BASE_URL")
    ...
    allowed_hosts, allowed_origins = mcp_transport_allowlists(settings.public_base_url)
...
app = _LazyWorkBuddyApp(create_workbuddy_yuncyb_app)
