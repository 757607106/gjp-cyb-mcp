...
from .models import YunCybDraft
from .ports import YunCybApiPort, YunCybReferenceSnapshot
...
YUNCYB_MCP_TOOL_NAMES = frozenset(
    {
        # 商品目录与基础资料
        "sync_products",
        "list_products",
        "search_products",
        "search_business_references",
        # 销售单
        "preview_sales_order",
        "submit_sales_order",
        "get_sales_order",
...
_TOOL_TITLES = {
    ...
    "search_products": "查找商品", "search_business_references": "查找基础资料",
...
_SEARCH_BUSINESS_REFERENCES_OUTPUT_SCHEMA = {
    "type": "object",
    "properties": {
        "ok": {"type": "boolean"},
        "error": _ERROR_OUTPUT_OBJECT,
...
_SEARCH_BUSINESS_REFERENCES_INPUT_SCHEMA = {
    "type": "object",
    "properties": {
        "reference_type": {
            "type": "string",
...
_PREVIEW_SALES_ORDER_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "customer": {
            "type": "string",
            "description": "生成就绪预览必需的客户名称、编号或候选ID；客户须由用户指定，ID取searchBusinessReferences或本工具客户候选，不得编造或擅用默认客户。空值提示缺失。",
        },
        "warehouse": {
            "type": "string",
            "description": "生成就绪预览必需的出库仓库名称、编号或候选ID；ID取searchBusinessReferences或本工具仓库候选，不得编造。未指定时仅采用唯一is_default=true项并标注默认，否则追问；空值不自动选仓库。",
        },
        "handler": {
            "type": "string",
            "description": "生成就绪预览必需的经手人名称、编号或候选ID；ID取searchBusinessReferences或本工具经手人候选，不得编造。未指定时仅采用唯一is_default=true项并标注默认，否则追问；空值不自动选人。",
        },
        "order_date": {
            "type": "string",
...
_LIST_SALES_ORDERS_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "customer_id": {
            "type": "string",
            "description": "客户内部ID，取searchBusinessReferences的customer候选或已核实销售单客户ID，不得编造或直接传客户名称；默认空不按客户筛选。",
        },
    },
}
...
_UPDATE_SALES_ORDER_INPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "handler_id": {
            "type": "string",
            "description": "经手人内部ID或唯一匹配的名称；ID取searchBusinessReferences的handler候选，不得编造；省略保留原值，不自动换成默认人员。",
        },
        "items": {
            "type": "array",
        ...
        "customer_id": {
            "type": "string",
            "description": "客户内部ID或唯一匹配名称，ID取searchBusinessReferences的customer候选，不得编造；默认空保留原值，客户由用户指定，已生效单据不可修改。",
        },
        "warehouse_id": {
            "type": "string",
            "description": "出库仓库内部ID或唯一匹配名称，ID取searchBusinessReferences的warehouse候选，不得编造；默认空保留原值，不自动换默认仓库，已生效单据不可修改。",
        },
        "save_type": {
            "type": "string",
            "enum": ["draft", "final"],
        ...
        "discount_account_id": {
            "type": "string",
            "description": "优惠结算账户内部ID，取searchBusinessReferences的settlement_account候选，不得编造；默认空保留原值。需选默认账户时仅取唯一is_default=true项并标注；已生效单据不可修改。",
        },
        "receipt_amount": {
            "type": "number",
        ...
        "receipt_account_id": {
            "type": "string",
            "description": "预收转正式时追加收款的结算账户内部ID，取searchBusinessReferences的settlement_account候选，不得编造；需选默认项时仅取唯一is_default=true项并标注，否则追问。默认空不传新账户。",
        },
        "confirmed_by_user": {
            "type": "boolean", "default": False,
...
class YunCybToolSet(QueryTools, DocumentTools, SessionToolSet):
    ...
    def __init__(
        ...
        super().__init__(
            [
                ...
                SessionFunctionTool(
                    self.search_business_references,
                    is_read_only=True,
                    output_schema=_SEARCH_BUSINESS_REFERENCES_OUTPUT_SCHEMA,
                    input_schema_override=_SEARCH_BUSINESS_REFERENCES_INPUT_SCHEMA,
                ),
                SessionFunctionTool(
            ...
            contexts=contexts,
            mcp_tool_names=YUNCYB_MCP_TOOL_NAMES,
        )
    ...
    async def search_products(self, keywords: list[str], limit: int = 10) -> dict[str, Any]:
        ...
        用listProducts，客户等基础资料用searchBusinessReferences，库存数量用queryStock。
    ...
    async def search_business_references(
        self,
        reference_type: str,
        keyword: str = "",
        limit: int = 5,
    ...
    async def preview_sales_order(
        ...
        searchBusinessReferences或searchProducts选择候选，再传完整整单重新预览。
    ...
    async def list_sales_orders(
        ...
        用户提供客户名称时先用searchBusinessReferences取得真实客户ID；其他筛选可
    ...
    @staticmethod
    def _unit_warnings(draft: YunCybDraft | None) -> list[dict[str, Any]]:
