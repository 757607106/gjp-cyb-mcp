...
基于 MCP Python SDK 的管家婆云创业版业务服务，对外服务名为 `yuncyb`，覆盖
销售、采购、库存、资金往来与报表的对话式业务场景，共 59 个工具。

```text
AI 平台 / SaaS 对话页 / WorkBuddy
  │ ERP Bearer / X-API-Key / MCP OAuth token
  ▼
管家婆云创业版 MCP（业务长期凭据：app；OAuth：workbuddy_app）
  ├─ McpIdentityResolver：解析身份与 YunCYB scopes
  ├─ McpToolSetResolver：按 tenant/account/session 隔离会话
  ├─ YunCybToolSet：资料追问、商品匹配、预览和提交
  └─ YunCybApiPort
...
[工具、API 与商品匹配](docs/architecture/yuncyb-tools-api-matching.md)。
...
具有 `yuncyb:write` 时才允许提交。`submit` 系列工具要求唯一
`idempotency_key`；当前实现提供会话内防重，生产多副本应接入共享幂等存储。
除销售单暴露 `save_type`（草稿 `0`、预收 `1`、正式 `2`）外，其余单据统一
`saveType=2` 保存过账，不暴露草稿态。

提示词只保留两个入口：`YUNCYB_MCP_INSTRUCTIONS` 由 MCP initialize 自动下发，
`YUNCYB_SYSTEM_PROMPT` 供 AI 平台装配 Agent，并复用前者后只补充跨工具确认、
...
```text
src/
├── yuncyb/  # ERP 业务领域、ToolSet、Port、Adapter、Prompt 和 MCP 入口
└── gjp_common/   # 身份、固定端点、凭据、MCP、配置和日志
integrations/workbuddy/  # WorkBuddy 连接器元数据与云创业版 Skill
```
...
- [AI 平台对接](docs/architecture/yuncyb-mcp-integration-guide.md)
