...
- 状态：已被 ADR 0002 替代；服务命名决策另被 ADR 0006 替代
- 日期：2026-08-04
...
4. 客户、出库仓库和经手人分别通过 `GET /customer/page`、
   ...
   `POST /sales/orders`。
5. ERP API URL 是部署级固定配置 `YUNCYB_BASE_URL`，不从会话、Token 登记
   请求或 Tool 参数读取。
...
7. 生产 MCP 不提供上游 Token 登记入口，不提供账号密码或验证码登录，也不接受
   动态 ERP URL。
8. 每个 `(tenant_id, account_id, session_id)` 使用隔离的 `YunCybToolSet`、
   `YunCybSession` 和内存商品目录。
9. 业务必填项是客户、出库仓库、经手人、录单日期和商品明细；备注可选。
   `id=0` 与 `saveType` 由工具管理，草稿/预收/正式分别映射 `0/1/2`。
10. 写单必须使用 `yuncyb:write`、当前不可变预览、用户明确确认和幂等键。
