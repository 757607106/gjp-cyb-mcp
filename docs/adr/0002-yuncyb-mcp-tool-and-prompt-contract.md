...
- 状态：已采纳；决策 1 的服务命名已被 ADR 0006 替代，十工具清单已被
  2026-09-20 五大业务域 59 工具扩展取代（框架同步迁移至 MCP Python SDK），
  鉴权、两段式确认与提示词契约仍然有效
- 日期：2026-08-27
...
1. MCP 服务名保持 `yuncyb`，发布以下十个工具：
   `syncProducts`、`listProducts`、`searchProducts`、
   `searchBusinessReferences`、`previewSalesOrder`、`submitSalesOrder`、
...
9. 提示词只保留两个入口：`YUNCYB_MCP_INSTRUCTIONS` 由 initialize 自动下发，
   `YUNCYB_SYSTEM_PROMPT` 供 AI 平台配置；不再拆分第三份响应契约。
...
14. 有显式 `inputSchema` 的工具不再在函数 docstring 重复维护 `Args:`；参数说明以
    Schema 为唯一来源。`YUNCYB_SYSTEM_PROMPT` 直接复用 MCP Instructions，
...
- 写操作仍要求 `yuncyb:write` 与明确用户确认；新增单据额外要求当前不可变预览和
  幂等键。
