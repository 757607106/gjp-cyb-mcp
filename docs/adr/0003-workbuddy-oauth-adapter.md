...
而现有 `erp_billing.app:app` 由对接方直接携带 ERP JWT 或 AI Token。
两种对接方式需要隔离，不应为 WorkBuddy 改动已有工具或运行入口。

## 决策

1. 新增独立组合入口 `erp_billing.workbuddy_app:app`；原有
   `erp_billing.app:app`、`BillingToolSet` 和 ERP Adapter 保持不变。
...
4. 单实例部署默认使用 Fernet 加密的 SQLite 状态存储；多副本部署前
   必须在组合根替换为共享存储。
5. API Key 联调连接器使用独立 `source=gjp-erp-billing-token`，
   不与正式 OAuth 连接器混用。
...
- 工具继续使用 `billing:read` / `billing:write`、当前预览和用户确认
  作为写操作的最终安全边界。
