...
ERP API URL 是部署级固定值 `YUNCYB_BASE_URL`，不随租户、请求或 Tool 调用
...
[工具、API 与商品匹配](yuncyb-tools-api-matching.md)。销售单核心工具：
...
| `searchBusinessReferences` | `reference_type` 为 customer/warehouse/handler/supplier/settlement_account |
...
- `YUNCYB_MCP_INSTRUCTIONS`：服务端自动放入 MCP initialize 响应，对接方无需
  复制或手工拼接。
- `YUNCYB_SYSTEM_PROMPT`：AI 平台 Agent 的补充系统提示词；复用 MCP
...
`searchBusinessReferences` 默认每页 5 条并返回 `page`、`page_size`、`total`、
...
class YunCybCredentialProvider:
...
http = BusinessAuthenticatedJsonClient(
    base_url=settings.yuncyb_base_url,
    credential_provider=YunCybCredentialProvider(),
)
```
...
- `yuncyb:write`、用户确认、当前预览和幂等键缺一不可。
- 多副本把预览与幂等结果迁移到共享存储。
