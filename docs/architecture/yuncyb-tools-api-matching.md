...
本文描述管家婆云创业版 MCP 服务的最终生产边界。业务方前端负责把文字、语音和图片整理成
...
flowchart LR
    ...
    Image["图片"] -->|VL 模型| Agent["YunCyb Agent"]
    ...
    Agent --> Options["searchBusinessReferences"]
...
多模态模型（VL）时，Agent 按 `YUNCYB_SYSTEM_PROMPT` 的图片处理规则直接
...
Token 结构和身份字段，再把 `yuncyb:read` / `yuncyb:write` 映射为不含凭据的
...
ERP 地址由部署环境 `YUNCYB_BASE_URL` 固定；同一 ERP Bearer 由凭据提供者
根据 `InvocationContext` 注入当前请求。URL 和 Bearer 均不进入工具参数、模型
上下文或工具结果。

`YunCybApiPort` 按业务域保留固定接口，全部以 `InvocationContext` 为第一参数：

```python
# 商品目录与基础资料
fetch_products(context, limit=None) -> YunCybProductSnapshot
search_customers / search_warehouses / search_staff
search_suppliers / search_settlement_accounts

# 销售单与销售退货
create_sales_order(context, payload) -> YunCybSalesOrderResult
get_sales_order_detail(context, order_id) -> YunCybSalesOrderDetailResult
search_sales_orders(context, ...) -> YunCybSalesOrderPageResult
void_sales_order(context, order_id) -> None
update_sales_order(context, order_id, payload) -> YunCybSalesOrderResult
...
的自动同步受 `YUNCYB_AUTO_SYNC_LIMIT`（缺省 10000）限制，避免超大
...
| `searchBusinessReferences` | `reference_type`、`keyword?`、`limit?`、`page?` | 查询客户、供应商、仓库、经手人或结算账户候选，支持翻页 | 分页元数据、名称、默认标记 |
...
`updateXxx` 具有 ERP 写副作用，要求 `yuncyb:write`、
明确用户确认；`submitXxx` 可省略幂等键（默认绑定 preview_id），预览提交成功后即失效，
不可用新幂等键重复提交。`getXxx` 和
`listXxx` 是只读操作，要求 `yuncyb:read`。服务不维护可逐行修改的文件草稿，
但会在隔离 Session 中短期保存不可变提交预览和成功幂等结果。

`listProducts`、`searchBusinessReferences` 和 `listSalesOrders` 统一返回
...
1. 从 `InvocationContext` 取得当前租户和账号，并校验 `yuncyb:read`。
2. 通过 `YunCybApiPort.fetch_products()` 调用当前 ERP 账号。
3. 归一化并过滤商品目录。
...
默认生鲜别名由 `YUNCYB_USE_DEFAULT_FRESH_ALIASES` 控制；外部文件通过
`YUNCYB_ALIAS_FILE`（即 `alias_path`）配置。加载时先加入默认别名，再加入
...
默认品类词由 `YUNCYB_USE_DEFAULT_CATEGORIES` 控制；外部文件通过
`YUNCYB_CATEGORY_FILE`（即 `category_path`）配置。加载时先加入默认品类词，
...
`YUNCYB_RECOMMENDATION_SCORE`（默认 `0.60`）时不会返回。
...
- `YUNCYB_PRODUCT_CATALOG`、`alias_path` 和 `category_path` 如有配置，仅作为服务端只读输入；
  模型和前端不能提供主机文件路径。
