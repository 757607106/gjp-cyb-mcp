...
生产 wheel 只包含 `src/yuncyb` 与 `src/gjp_common`：

```bash
uv build --wheel
uv pip install dist/yuncyb_mcp-*.whl
```
...
| 模式 | ASGI 入口 | 鉴权 | 默认端口 |
|---|---|---|---|
| 生产直连 | `yuncyb.app:app` | ERP Bearer Token / `X-API-Key` | `8102` |
| 生产 OAuth | `yuncyb.workbuddy_app:app` | OAuth 2.1，服务端绑定 ERP Token | `8103` |

共同配置：

- `GJP_ENV=production`：加载生产配置并启用生产约束；
- `YUNCYB_BASE_URL`：部署级固定 HTTPS 地址；
- `YUNCYB_TIMEOUT_SECONDS`：ERP 请求超时，默认 30 秒；
- `YUNCYB_CATALOG_TTL_SECONDS`：租户商品目录 TTL，默认 600 秒。
...
客户端时，也必须通过 `create_yuncyb_mcp_service(..., shutdown=...)` 释放资源。
...
- 写工具必须有 `yuncyb:write`、有效预览、明确确认和幂等键。

工具参数和销售单业务契约属于架构文档，不在部署文档重复维护，参见
[工具与 API 匹配](../architecture/yuncyb-tools-api-matching.md) 和
